    ...
    <div className="glass-surface rounded-xl p-3 flex flex-col gap-2">
      {editing ? (
        <>
          ...
          <select value={nextSteps} onChange={e => setNS(e.target.value as NextStep)} style={inputStyle}>
            {(Object.keys(NEXT_STEP_LABELS) as NextStep[]).map(k => (
              <option key={k} value={k}>{NEXT_STEP_LABELS[k]}</option>
            ))}
          </select>
          ...
          <div className="flex gap-2">
            <button onClick={saveEdit} disabled={saving} className="flex-1 text-xs py-1 rounded font-semibold disabled:opacity-40" style={{ backgroundColor: '#b87333', color: '#0f0f0f' }}>
              {saving ? '…' : 'Save'}
            </button>
            <button onClick={() => { setEditing(false); setProject(log.project); setNS(log.next_steps); setNotes(log.notes ?? ''); }} className="flex-1 text-xs py-1 rounded border border-primary/30 text-muted-foreground hover:bg-primary/10">
              Cancel
            </button>
          </div>
        </>
        ...
        <>
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm font-semibold leading-tight" style={{ color: '#d4a373' }}>{log.project}</p>
            <div className="flex gap-1 shrink-0">
              <button onClick={() => setEditing(true)} title="Edit" className="text-[10px] px-1.5 py-0.5 rounded border border-primary/20 text-muted-foreground hover:text-primary hover:border-primary/50 transition-colors">✎</button>
              {deleting ? (
                <>
                  <button onClick={doDelete} disabled={saving} className="text-[10px] px-1.5 py-0.5 rounded bg-red-900/60 text-red-400 border border-red-700/50 hover:bg-red-900 disabled:opacity-40">{saving ? '…' : 'Yes'}</button>
                  <button onClick={() => setDeleting(false)} className="text-[10px] px-1.5 py-0.5 rounded border border-primary/20 text-muted-foreground hover:bg-primary/10">No</button>
                </>
              ) : (
                <button onClick={() => setDeleting(true)} title="Archive" className="text-[10px] px-1.5 py-0.5 rounded border border-primary/20 text-muted-foreground hover:text-red-400 hover:border-red-700/50 transition-colors">✕</button>
              )}
            </div>
          </div>
          <span className="self-start text-[11px] font-medium px-2 py-0.5 rounded" style={{ backgroundColor: color.bg, color: color.text }}>
            {NEXT_STEP_LABELS[log.next_steps] ?? log.next_steps}
          </span>
          {log.notes && <p className="text-xs text-muted-foreground leading-relaxed line-clamp-3">{log.notes}</p>}
        </>
      )}
    </div>
  );
...
function KanbanColumn({
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-2 py-1 rounded" style={{ backgroundColor: HEADER_COLOR[status] }}>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-red-900/40 bg-red-950/20 p-4 text-red-400 text-sm">{error}</div>
        ) : (
