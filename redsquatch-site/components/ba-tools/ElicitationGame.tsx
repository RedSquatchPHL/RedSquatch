...
import { ELICITATION_ROUNDS, type ElicitationOption } from '@/lib/ba-content';
...
export default function ElicitationGame() {
  const [progress, setProgress] = useState<Progress | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [roundIndex, setRoundIndex] = useState(0);
  const [pickedId, setPickedId] = useState<string | null>(null);

  const round = ELICITATION_ROUNDS[roundIndex % ELICITATION_ROUNDS.length];
  const options = useMemo(() => shuffle(round.options), [round.id]);
  const picked = options.find(o => o.id === pickedId) ?? null;
  const correctOption = options.find(o => o.correct) as ElicitationOption;

  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${API}/api/client/ba-tools/elicitation-technique`, { credentials: 'include' });
  ...
  async function pick(option: ElicitationOption) {
    if (pickedId) return;
    setPickedId(option.id);
    try {
      const res = await fetch(`${API}/api/client/ba-tools/elicitation-technique`, {
        method: 'PUT',
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <span className="text-xs" style={{ color: 'rgba(255,255,255,0.55)' }}>
          Pick the elicitation technique that actually fits the situation — the others are real techniques too, just mismatched here.
        </span>
        {progress && (
              ...
              <div className="flex items-start gap-2">
                {revealed && (option.correct
                  ...
                  : isPicked
                    ...
                    : <span className="w-4" />)}
                <span className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.85)' }}>{option.technique}</span>
              </div>
      ...
      {pickedId && (
        <div className="flex items-center justify-between">
          <span className="text-xs" style={{ color: picked?.correct ? '#8fce8f' : '#e08787' }}>
            {picked?.correct ? 'Correct!' : `Not quite — "${correctOption.technique}" was the better fit.`}
          </span>
          <button
            onClick={nextRound}
