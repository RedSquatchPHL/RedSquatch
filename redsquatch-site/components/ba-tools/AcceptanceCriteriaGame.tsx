...
import { AC_ROUNDS, type ACOption } from '@/lib/ba-content';
...
export default function AcceptanceCriteriaGame() {
  const [progress, setProgress] = useState<Progress | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [roundIndex, setRoundIndex] = useState(0);
  const [pickedId, setPickedId] = useState<string | null>(null);

  const round = AC_ROUNDS[roundIndex % AC_ROUNDS.length];
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${API}/api/client/ba-tools/acceptance-criteria`, { credentials: 'include' });
  ...
  async function pick(option: ACOption) {
    if (pickedId) return;
    setPickedId(option.id);
    try {
      const res = await fetch(`${API}/api/client/ba-tools/acceptance-criteria`, {
        method: 'PUT',
